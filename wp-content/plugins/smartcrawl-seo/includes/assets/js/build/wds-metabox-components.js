(function (wp, _, $, Wds) {
  ...
  function (_EventTarget) {
    ...
    _createClass(ClassicEditor, [{
      ...
      value: function get_data() {
        ...
        return post.set_id(data.post_id).set_type(data.post_type).set_author_id(data.post_author).set_title(data.post_title).set_content(data.content).set_excerpt(data.excerpt).set_category_ids(data.catslist.split(',')).set_tag_ids(tags ? tags.split(',') : []).set_slug(data.post_name).set_date(new Date()).set_modified(new Date()).set_permalink(Config_Values.get('home_url', 'metabox') + '?p=' + post.get_id());
      }
    }, {
    ...
    _createClass(String_Utils, null, [{
      ...
      value: function remove_shortcodes(string) {
        ...
        return string.replace(regex, function (match, start_bracket, shortcode, attributes, match_4, content, end_bracket, offset, string) {
          ...
          var omitted = Config_Values.get('omitted_shortcodes', 'metabox');
    ...
    _createClass(MacroReplacement, [{
      ...
      value: function get_trimmed_excerpt(excerpt, content) {
        var string = !!excerpt ? excerpt : content,
            metadesc_max_length = Config_Values.get('metadesc_max_length', 'metabox'); // Strip all HTML tags
      ...
      value: function replace(text, post) {
        ...
        var promises = [];
        Object.keys(load_required).forEach(function (macro) {
          var callback = load_required[macro],
        ...
        return new Promise(function (resolve, reject) {
          Promise.all(promises).then(function (loaded) {
            loaded.push(_this.get_replacements(post));
      ...
      key: "get_replacements",
      value: function get_replacements(post) {
        var dateFormat = Config_Values.get('date_format', 'metabox'),
  ...
  function (_EventTarget) {
    ...
    _createClass(MetaboxOnpage, [{
      ...
      value: function init() {
        ...
        }, 200)).on('input propertychange', '.wds-meta-field', _.debounce(function (e) {
  ...
  (function () {
    ...
    window.Wds.macroReplacement = macroReplacement;
